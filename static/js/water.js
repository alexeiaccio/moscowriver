/******/ (function(modules) {
	 ...
	 */
					...
					*/
										var backwards = this._yoyo && (prevCycle & 1) !== 0,
											wrap =
												backwards === (this._yoyo && (this._cycle & 1) !== 0),
								...
								for (i = 0; i < l; i++) {
									...
									if (correlate) {
										...
										m1 =
											p2 -
											(p2 - p1) *
												(basic ? curviness * 0.5 : r1 !== 0 ? tl / r1 : 0);
										m2 =
											p2 +
											(p3 - p2) *
												(basic ? curviness * 0.5 : r2 !== 0 ? tl / r2 : 0);
										mm =
											p2 -
							...
							BezierPlugin = _gsScope._gsDefine.plugin({
								...
								set: function(v) {
									...
									while (--i > -1) {
										...
										val =
											(t * t * b.da + 3 * inv * (t * b.ca + inv * b.ba)) * t +
											b.a;
										if (this._round[p]) {
											val = Math.round(val);
										}
	 ...
	 */
						...
						function(TweenPlugin, TweenLite) {
							...
							var _numExp = /(?:\-|\.|\b)(\d|\.|e\-)+/g,
								...
								_supportsOpacity = (function() {
									...
									_isSafariLT6 =
										_isSafari &&
										Number(_agent.substr(_agent.indexOf("Version/") + 8, 1)) <
											6;
								...
								_convertToPixels = (_internals.convertToPixels = function(
									...
									if (sfx === "%" && p.indexOf("border") !== -1) {
										...
										if (
											...
											style[horiz ? "borderLeftWidth" : "borderTopWidth"] =
												v + sfx;
										}
										...
										if ((i = cs.length)) {
											while (--i > -1) {
												...
												if (
													...
													s[
														p.replace(_camelExp, _camelFunc)
													] = cs.getPropertyValue(p);
												}
											}
										} else {
				 ...
				 */
								_getDimension = function(t, p, cs) {
									...
									while (--i > -1) {
										v -=
											parseFloat(_getStyle(t, "padding" + a[i], cs, true)) || 0;
										v -=
											parseFloat(
			 ...
			 */
							...
							var _transformProps = "scaleX,scaleY,scaleZ,x,y,z,skewX,skewY,rotation,rotationX,rotationY,perspective,xPercent,yPercent".split(
								...
								_forceSVGTransformAttr = (function() {
									...
									var force =
											_ieVers || (/Android/i.test(_agent) && !window.chrome),
										svg,
										rect,
										width;
								...
								_getTransform = (_internals.getTransform = function(
									...
									if (m !== _identity2DMatrix) {
										if (m.length === 16) {
											...
											tm.scaleX =
												((Math.sqrt(a11 * a11 + a21 * a21) * rnd + 0.5) | 0) /
												rnd;
											tm.scaleY =
												((Math.sqrt(a22 * a22 + a23 * a23) * rnd + 0.5) | 0) /
												rnd;
											tm.scaleZ =
												((Math.sqrt(a32 * a32 + a33 * a33) * rnd + 0.5) | 0) /
												rnd;
											tm.skewX =
												a12 || a22
											...
											if (tm.svg) {
												tm.x -=
													tm.xOrigin - (tm.xOrigin * a11 - tm.yOrigin * a12);
												tm.y -=
													tm.yOrigin - (tm.yOrigin * a21 - tm.xOrigin * a22);
											}
										} else if (
											(!_supports3D ||
												parse ||
								...
								_setIETransformRatio = function(v) {
									...
									if (!clip) {
										...
										t.ieOffsetX = Math.round(
											(w - ((a < 0 ? -a : a) * w + (b < 0 ? -b : b) * h)) / 2 +
												ox
										);
										t.ieOffsetY = Math.round(
											(h - ((d < 0 ? -d : d) * h + (c < 0 ? -c : c) * w)) / 2 +
												oy
										);
										for (i = 0; i < 4; i++) {
											prop = _margins[i];
									...
									parser: function(t, e, p, cssp, pt, plugin, vars) {
										...
										if (orig || (_supports3D && has3D && m1.zOrigin)) {
											...
											if (_transformProp) {
												...
												orig =
													(orig || _getStyle(t, p, _cs, false, "50% 50%")) + ""; //cast as string to avoid errors
												pt = new CSSPropTween(
													style,
													p,
													0,
								...
								for (p in vars) {
									...
									if (sp) {
										...
										if (
											...
											es =
												en || en === 0 ? (rel ? en + bn : en) + esfx : vars[p]; //ensures that any += or -= prefixes are taken care of. Record the end value before normalizing the suffix because we always want to end the tween on exactly what they intended even if it doesn't match the beginning value's suffix.
												...
												if (esfx !== "")
													if (en || en === 0)
														if (bn) {
															...
															if (esfx === "%") {
																bn /=
																	_convertToPixels(target, p, 100, "%") / 100;
							...
							var w = _gsScope.GreenSockGlobals || _gsScope,
								...
								Back = _wrap(
									...
									_createBack("BackOut", function(p) {
										return (
											(p = p - 1) * p * ((this._p1 + 1) * p + this._p1) + 1
										);
									}),
							...
							RoughEase = _class(
								...
								function(vars) {
									...
									var taper = vars.taper || "none",
										...
										template =
											vars.template instanceof Ease ? vars.template : null,
										strength =
							...
							_createElastic = function(n, f, def) {
								var C = _class(
										...
										function(amplitude, period) {
											...
											this._p2 =
												(period || def) / (amplitude < 1 ? amplitude : 1);
											this._p3 =
												this._p2 / _2PI * (Math.asin(1 / this._p1) || 0);
	 ...
	 */
				(function(window, moduleName) {
					"use strict";
					var _globals = (window.GreenSockGlobals =
	 ...
	 * ----------------------------------------------------------------
	 */
					var EventDispatcher = _class("events.EventDispatcher", function(
						target
					) {
						this._listeners = {};
	 ...
	 */
					...
					p.progress = p.totalProgress = function(value, suppressEvents) {
						...
						return !arguments.length
							? duration ? this._time / duration : this.ratio
	 ...
	 */
					...
					p.render = function(time, suppressEvents, force) {
						...
						while (tween) {
							...
							if (
								tween._active ||
								(time >= tween._startTime && !tween._paused)
							) {
								if (!tween._reversed) {
									tween.render(
										(time - tween._startTime) * tween._timeScale,
